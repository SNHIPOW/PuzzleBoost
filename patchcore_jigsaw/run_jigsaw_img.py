...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        with device_context:
            ...
            ###            
            projection = methods["get_projection"](device)
            jigsawcls = methods["get_jigsawcls"](device)
            projection.train()
            jigsawcls.train()
            
            result_txt = f'{log_group}.txt'
            jigsaw_cls_num = jigsawcls._cls_num()
            criterion = nn.CrossEntropyLoss(reduction='mean')
            optimizer_p = optim.Adam(params=projection.parameters(), lr=1e-4)
            optimizer_j = optim.Adam(params=jigsawcls.parameters(), lr=1e-4)
            
            PatchCore_list = methods["get_patchcore"](imagesize, sampler, projection, device)
            ###
            ...
            for i_epoch in range(epochs): 
                ...
                ###########################
                with tqdm.tqdm(
                        dataloaders["jigsaw_training"], desc=f"Epoch:{i_epoch}, jigsaw_training ", position=1, leave=False
                    ) as data_iterator:
...
def patch_core(
    ...
    def get_patchcore(input_shape, sampler, projection, device):
        loaded_patchcores = []
